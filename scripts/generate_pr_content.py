...
def format_pr_body(changes, format_type="github"):
    ...
    if format_type == "github":
        ...
        if changes['updated_datasets'] > 0:
            ...
            for dataset in changes['updated_dataset_list'][:10]:
                body += f"| {dataset['name']} | {dataset['prev_count']} | {dataset['curr_count']} | +{dataset['diff']} |\n"
            
            if len(changes['updated_dataset_list']) > 10:
                body += f"\n... and {len(changes['updated_dataset_list']) - 10} more datasets updated\n"
            body += "\n"
...
def write_github_output(title, body):
...
def main():
    ...
    # Format PR body
    pr_body = format_pr_body(changes, args.format)
    ...
    write_github_output(pr_title, pr_body)
