...
## Key Data Formats
- **Citation JSON** (`citations/json_opencite/`), the canonical output. Schema v2: top-level `dataset_id`, `num_citations`, `date_last_updated`, `citation_details[]`, `metadata.{schema_version="2.0", discovery_backend="opencite", fetch_status, anchor_count, anchor_errors, anchor_judgment_model, context_anchors[], ...}`; per-citation `source_doi` + `source_relation` (one of `References`, `IsDerivedFrom`, `IsIdenticalTo`, `IsVersionOf`). `metadata.context_anchors[]` carries the anchors that phase 3 bucketed as `umbrella` / `methodology` / `related_work` / `irrelevant` (each entry: `anchor_identifier`, `anchor_identifier_type`, `source_relation`, `classification`, `paper_title`, `reason`).
