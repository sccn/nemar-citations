...
class TestCitationUtils(unittest.TestCase):
    ...
    def test_safe_get_value_functions(self):
        ...
        self.assertEqual(citation_utils._safe_get_value_from_dict(test_dict, 'present'), 'value')
        self.assertEqual(citation_utils._safe_get_value_from_dict(test_dict, 'empty'), 'n/a')
        self.assertEqual(citation_utils._safe_get_value_from_dict(test_dict, 'none_val'), 'n/a')
        self.assertEqual(citation_utils._safe_get_value_from_dict(test_dict, 'missing'), 'n/a')
    ...
    def test_get_citation_summary_from_json(self):
        ...
        # Save a test file
        dataset_id = 'test_summary'
        saved_filepath = citation_utils.save_citation_json(
            dataset_id, self.sample_citations_df, self.test_dir
        )
        
        # Get summary
        summary = citation_utils.get_citation_summary_from_json(saved_filepath)
        
        expected_keys = ['dataset_id', 'num_citations', 'total_cumulative_citations', 'date_last_updated']
        for key in expected_keys:
            self.assertIn(key, summary)
        
        self.assertEqual(summary['dataset_id'], dataset_id)
        self.assertEqual(summary['num_citations'], 2)
        self.assertEqual(summary['total_cumulative_citations'], 15)  # 10 + 5
        self.assertIsNotNone(summary['date_last_updated'])
    
    def test_migrate_pickle_to_json_functionality(self):
        """Test pickle to JSON migration functionality."""
        # Create a test pickle file
        pickle_file = os.path.join(self.test_dir, 'test_dataset.pkl')
        self.sample_citations_df.to_pickle(pickle_file)
        
        # Migrate to JSON
        json_filepath = citation_utils.migrate_pickle_to_json(
            pickle_file, self.test_dir, 'test_dataset'
        )
        
        expected_json_path = os.path.join(self.test_dir, 'test_dataset_citations.json')
        self.assertEqual(json_filepath, expected_json_path)
        self.assertTrue(os.path.exists(json_filepath))
        
        # Verify content
        loaded_data = citation_utils.load_citation_json(json_filepath)
        self.assertEqual(loaded_data['dataset_id'], 'test_dataset')
        self.assertEqual(loaded_data['num_citations'], 2)
    
    def test_migrate_pickle_to_json_auto_dataset_id(self):
        """Test pickle migration with automatic dataset ID extraction."""
        # Create pickle file with dataset ID in name
        pickle_file = os.path.join(self.test_dir, 'ds000123.pkl')
        self.sample_citations_df.to_pickle(pickle_file)
        
        # Migrate without explicit dataset_id
        json_filepath = citation_utils.migrate_pickle_to_json(pickle_file, self.test_dir)
        
        # Verify dataset ID was extracted correctly
        loaded_data = citation_utils.load_citation_json(json_filepath)
        self.assertEqual(loaded_data['dataset_id'], 'ds000123')
    ...
    def test_with_real_citation_files(self):
        ...
        if not self.test_data_dir.exists():
            self.skipTest("Real citation data directory not found")
        
        # Find a small citation file to test with
        json_files = list(self.test_data_dir.glob('ds005410_citations.json'))
        if not json_files:
            self.skipTest("No test citation files found")
        
        test_file = json_files[0]
        
        # Test loading real file
        loaded_data = citation_utils.load_citation_json(str(test_file))
        
        # Verify structure
        required_keys = ['dataset_id', 'num_citations', 'date_last_updated', 'metadata', 'citation_details']
        for key in required_keys:
            self.assertIn(key, loaded_data)
        
        # Test summary extraction
        summary = citation_utils.get_citation_summary_from_json(str(test_file))
        self.assertEqual(summary['dataset_id'], loaded_data['dataset_id'])
        self.assertEqual(summary['num_citations'], loaded_data['num_citations'])
        
        # Verify citation details structure if present
        if loaded_data['citation_details']:
            first_citation = loaded_data['citation_details'][0]
            required_citation_keys = ['title', 'author', 'venue', 'year', 'url', 'cited_by']
            for key in required_citation_keys:
                self.assertIn(key, first_citation)
