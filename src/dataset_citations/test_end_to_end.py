...
def test_minimal_workflow(test_datasets: List[str] = None, output_dir: str = None):
    ...
    try:
        ...
        if not test_proxy_setup():
            logger.warning(
                "Proxy setup test failed - this may be expected in test environments"
            )
...
def test_citation_retrieval_with_secrets(
    test_datasets: List[str], output_dir: str
) -> bool:
    ...
    try:
        ...
        for dataset_id in test_datasets[
            :1
        ]:  # Test only first dataset to save API calls
            ...
            if citation_count > 0:
                ...
                if not citations_df.empty:
                    ...
                    citation_utils.save_citation_json(
                        dataset_id, citations_df, output_dir
                    )

                    logger.info(
                        f"✓ Successfully retrieved and saved citations for {dataset_id}"
                    )
                else:
...
def test_json_operations(test_datasets: List[str], output_dir: str) -> bool:
    ...
    try:
        ...
        for json_file in json_files:
            ...
            # Test loading
            citation_utils.load_citation_json(str(json_file))
