...
def generate_citations_csv(
    ...
    citations_series = pd.Series(
        citation_counts, name="number_of_citations", dtype="float64"
    )
...
def generate_updated_datasets_csv(
    ...
    updated_df = pd.DataFrame.from_dict(
        citation_counts, orient="index", columns=["retrieved_citations_count"]
    )
...
Examples:
  ...
  python regenerate_csv_from_json.py --json-dir citations/json_opencite --output-dir citations/ --update-previous
    ...
    logger.info(
        f"Found {len(citation_counts)} datasets with {total_citations} total citations"
    )

    # Generate CSV files
    citations_csv = generate_citations_csv(
        citation_counts, args.output_dir, args.date_suffix
    )
    updated_csv = generate_updated_datasets_csv(
        citation_counts, args.output_dir, args.date_suffix
    )
