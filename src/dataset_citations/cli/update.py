...
def load_input_data(
    ...
    try:
        ...
        if not datasets:
            ...
            return None, None
        logger.info(
            f"Successfully read {len(datasets)} dataset IDs from {dataset_list_file_path}"
        )
        ...
        logger.error(
            f"Could not read dataset list file: {dataset_list_file_path}. Error: {e}"
        )
        return None, None

    try:
        num_cites_old_df = pd.read_csv(
            previous_citations_file_path, index_col="dataset_id"
        )
        ...
        logger.error(
            f"Could not read previous citations file: {previous_citations_file_path}. Error: {e}"
        )
        return (
            datasets,
            None,
        )  # Return datasets if successfully loaded, but num_cites_old failed

    return datasets, num_cites_old


def fetch_citation_count(
    dataset_id: str, num_cites_old_series: pd.Series
) -> tuple[str, int]:
    ...
    try:
        ...
        logger.error(
            f"Unexpected error calling gc.get_citation_numbers for {dataset_id}. Error: {e}"
        )
...
def update_citation_counts(
    ...
    num_cites_new_dict = {}
    logger.info(
        f"Updating citation numbers for {len(datasets)} datasets using {max_workers} workers..."
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_dataset)):
            ...
            try:
                ...
                logger.info(
                    f"Completed fetching for {dataset_id} ({i + 1}/{len(datasets)}). Count: {count}"
                )
            except Exception as exc:
                logger.error(
                    f"{dataset_id} generated an exception during fetch_citation_count: {exc}"
                )
                num_cites_new_dict[dataset_id] = num_cites_old.get(
                    dataset_id, 0
                )  # Fallback

    num_cites_new = pd.Series(
        num_cites_new_dict, name="number_of_citations", dtype="float64"
    )
    num_cites_new_aligned, num_cites_old_aligned = num_cites_new.align(
        num_cites_old, join="outer", fill_value=0
    )
    ...
    if update_flag:
        ...
        for dataset_id_changed, diff_value in changed_counts.items():
            ...
            logger.info(
                f"  Dataset {dataset_id_changed}: old={old_val}, new={new_val}, diff={diff_value}"
            )

        datasets_updated_for_counts = num_cites_diff[num_cites_diff > 0].index.tolist()
        newly_added_datasets = num_cites_new.index.difference(
            num_cites_old.index
        ).tolist()
        ...
        if not datasets_updated_for_counts:
            logger.info(
                "No datasets found with an increase in citations. Update flag might be due to decreases."
            )
    else:
...
def fetch_detailed_citations_for_dataset(
    ...
    logger.info(
        f"Fetching detailed citation list for {dataset_id} ({num_citations_to_fetch} citations)..."
    )
    try:
        ...
        if citations_df is not None and not citations_df.empty:
            return (
                ...
                citations_df,
                None,
            )  # dataset_id, dataframe, error_message
            ...
            logger.info(
                f"No citations retrieved by gc.get_citations for {dataset_id} (returned empty)."
            )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for d in datasets_to_process:
            ...
            if pd.isna(current_num_cites):
                logger.warning(
                    f"Skipping detailed citation list for {d} as its citation count is NaN."
                )
                continue
            if int(current_num_cites) == 0:
                logger.info(
                    f"Skipping detailed citation list for {d} as it has 0 citations."
                )
                successful_updates_details[d] = (
                    0  # Consider 0 citations as a successful (empty) update
                )
                continue
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_dataset)):
            ...
            try:
                ...
                if error_message:
                    logger.error(
                        f"Error fetching detailed citations for {dataset_id}: {error_message}"
                    )
                ...
                if citations_df is not None and not citations_df.empty:
                    ...
                    if output_format in ["pickle", "both"]:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to save pickle for {dataset_id} to {output_pkl_path}. Error: {e_save}"
                            )
                    ...
                    if output_format in ["json", "both"]:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to save JSON for {dataset_id}. Error: {e_save}"
                            )
                    ...
                    successful_updates_details[dataset_id] = (
                        0  # Explicitly state 0 retrieved
                    )
                ...
                logger.error(
                    f"{dataset_id} generated an exception during detailed citation processing: {exc}"
                )
                unsuccessful_list_update.append(dataset_id)
...
def save_updated_dataset_summary(
    successful_updates_details: dict, output_dir: str
) -> str | None:
    ...
    try:
        pd.Series(successful_updates_details).to_csv(
            filepath,
            index=True,
            header=["retrieved_citations_count"],
        ...
        logger.error(
            f"Failed to save updated_datasets summary to {filepath}. Error: {e}"
        )
        return None
...
def main():
    ...
    parser.add_argument(
        "--no-update-num-cites",
        action="store_false",
        dest="update_num_cites",
        help="Skip updating citation numbers.",
    )
    parser.add_argument(
        "--no-update-cite-list",
        action="store_false",
        dest="update_cite_list",
    ...
    datasets, num_cites_old = load_input_data(
        args.dataset_list_file, args.previous_citations_file
    )
    ...
    overall_update_occurred = (
        False  # Tracks if any significant update happened for lists
    )
    datasets_for_list_update = []  # Initialize with an empty list

    if args.update_num_cites:
        num_cites_new_res, _, datasets_with_new_counts, counts_updated_flag = (
            update_citation_counts(
                datasets,
                num_cites_old,
                args.workers,  # Pass workers argument
            )
        )
        ...
        logger.info(
            "Skipping citation number updates. Will update detailed lists for all datasets."
        )
        datasets_for_list_update = datasets  # Process all datasets for lists

    if args.update_cite_list and datasets_for_list_update:
        logger.info(
            f"Updating detailed citation lists for {len(datasets_for_list_update)} dataset(s)."
        )
        ...
        if successful_details:
            ...
            overall_update_occurred = (
                True  # If any list was successfully processed/saved
            )
        if unsuccessful_details:
            logger.warning(
                f"Failed to update detailed citation lists for: {unsuccessful_details}"
            )
    ...
    if overall_update_occurred:
        ...
        logger.info(
            "Update process completed. No changes were made to citation counts or lists."
        )
