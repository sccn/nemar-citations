...
def migrate_single_file(
    pickle_filepath: str, output_dir: str, overwrite: bool = False
) -> bool:
    """
    ...
    try:
        ...
        if os.path.exists(json_filepath) and not overwrite:
            logger.info(
                f"JSON file already exists for {dataset_id}, skipping (use --overwrite to force)"
            )
            return True
        ...
        json_filepath = citation_utils.save_citation_json(
            dataset_id, citations_df, output_dir
        )
...
def generate_migration_report(
    input_dir: str, output_dir: str, successful: list, failed: list
):
    """
    ...
    if successful:
        report_lines.extend(["Successfully Migrated Datasets:", "-" * 30])
        for dataset_id in successful:
            report_lines.append(f"  ✓ {dataset_id}")
        report_lines.append("")

    if failed:
        report_lines.extend(["Failed Migrations:", "-" * 20])
    ...
    if validation_results["missing_json_files"]:
        report_lines.extend(["Missing JSON Files:", "-" * 20])
        for dataset_id in validation_results["missing_json_files"]:
            report_lines.append(f"  - {dataset_id}")
        report_lines.append("")

    if validation_results["validation_errors"]:
        report_lines.extend(["Validation Errors:", "-" * 20])
